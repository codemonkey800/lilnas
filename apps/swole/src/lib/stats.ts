...
export type ExerciseGroup = { exercise: ExerciseRow; logs: SetLogRow[] }

/**
 * Sums weight × actualReps over rows where both are non-null (naturally
 * selects weighted sets that recorded work; bodyweight/time-based/cardio
 * carry null weight; skipped weighted sets carry null actualReps).
 */
export function weightedVolume(setLogs: SetLogRow[]): number {
  let total = 0
  for (const log of setLogs) {
    if (log.weight !== null && log.actualReps !== null) {
      total += log.weight * log.actualReps
    }
  }
  return total
}
...
/**
 * Groups set logs by exercise, preserving routine order (exercises is
 * already ordered by orderInRoutine). Exercises with zero logs are omitted.
 * Each exercise's logs are in chronological order (preserving input order).
 */
export function groupSetLogsByExercise(
  setLogs: SetLogRow[],
  exercises: ExerciseRow[],
): ExerciseGroup[] {
  const byExercise = new Map<number, SetLogRow[]>()
  for (const log of setLogs) {
    const group = byExercise.get(log.exerciseId) ?? []
    group.push(log)
    byExercise.set(log.exerciseId, group)
  }
