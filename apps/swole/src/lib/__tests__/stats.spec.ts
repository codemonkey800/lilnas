...
function makeExerciseRow(
  overrides: Partial<ExerciseRow> & { id: number },
): ExerciseRow {
  return {
    routineId: 1,
    name: 'Exercise',
...
function makeSetLogForExercise(
  exerciseId: number,
  id: number,
  overrides: Partial<SetLogRow> = {},
): SetLogRow {
...
describe('weightedVolume', () => {
  it('sums weight × actualReps for weighted sets', () => {
    const logs = [
      makeSetLogForExercise(1, 1, { weight: 100, actualReps: 10 }),
      makeSetLogForExercise(1, 2, { weight: 100, actualReps: 8 }),
      makeSetLogForExercise(1, 3, { weight: 105, actualReps: 10 }),
    ]
    expect(weightedVolume(logs)).toBe(100 * 10 + 100 * 8 + 105 * 10)
  })

  it('includes Failed weighted sets with weight and actualReps', () => {
    const logs = [
      makeSetLogForExercise(1, 1, {
        weight: 100,
        actualReps: 6,
        action: 'Failed',
      }),
    ]
    expect(weightedVolume(logs)).toBe(600)
  })
  ...
  it('excludes bodyweight/cardio rows with null weight', () => {
    const logs = [makeSetLogForExercise(1, 1, { weight: null, actualReps: 15 })]
    expect(weightedVolume(logs)).toBe(0)
  })

  it('excludes skipped weighted rows with null actualReps', () => {
    const logs = [
      makeSetLogForExercise(1, 1, { weight: 100, actualReps: null }),
    ]
    expect(weightedVolume(logs)).toBe(0)
  })

  it('sums only weighted rows in a mixed array', () => {
    const logs = [
      makeSetLogForExercise(1, 1, { weight: 100, actualReps: 10 }),
      makeSetLogForExercise(2, 2, { weight: null, actualReps: 15 }),
    ]
    expect(weightedVolume(logs)).toBe(1000)
  })
})
...
describe('groupSetLogsByExercise', () => {
  it('groups logs interleaved across two exercises in routine order', () => {
    const ex1 = makeExerciseRow({ id: 1, orderInRoutine: 0 })
    const ex2 = makeExerciseRow({ id: 2, orderInRoutine: 1 })
    const log1 = makeSetLogForExercise(1, 1)
    const log2 = makeSetLogForExercise(2, 2)
    const log3 = makeSetLogForExercise(1, 3)
    const result = groupSetLogsByExercise([log1, log2, log3], [ex1, ex2])
  ...
  it('omits exercises with zero logs', () => {
    const ex1 = makeExerciseRow({ id: 1, orderInRoutine: 0 })
    const ex2 = makeExerciseRow({ id: 2, orderInRoutine: 1 })
    const log1 = makeSetLogForExercise(1, 1)
    const result = groupSetLogsByExercise([log1], [ex1, ex2])
    expect(result).toHaveLength(1)
    expect(result[0]!.exercise.id).toBe(1)
  })

  it('includes archived exercise rows that have logs', () => {
    const archived = makeExerciseRow({
      id: 3,
      orderInRoutine: 0,
      archivedAt: new Date('2025-01-01'),
    })
    const log = makeSetLogForExercise(3, 1)
    const result = groupSetLogsByExercise([log], [archived])
    expect(result).toHaveLength(1)
    expect(result[0]!.exercise.id).toBe(3)
  })

  it('returns empty array for empty inputs', () => {
    expect(groupSetLogsByExercise([], [])).toEqual([])
    const ex = makeExerciseRow({ id: 1, orderInRoutine: 0 })
    expect(groupSetLogsByExercise([], [ex])).toEqual([])
  })
})
