...
export function mapDeleteSessionError(result: ActionError): ErrorToast {
  if (result.code === 'SessionHasProgression') {
    return {
      message: "This session recorded a progression and can't be deleted.",
      severity: 'error',
    }
  }
  if (result.code === 'SessionNotCompleted') {
    return {
      message: 'Only completed sessions can be deleted.',
      severity: 'error',
    }
  }
  if (result.code === 'NotFoundError') {
    return { message: 'Session not found.', severity: 'warning' }
  }
  return { message: 'Could not delete session. Try again.', severity: 'error' }
}
...
// ---------------------------------------------------------------------------
// Session-detail formatters
// ---------------------------------------------------------------------------

/**
 * Formats the wall-clock duration of a session.
 * "52s" for sub-minute, "47m" for < 1h, "1h 12m" for ≥ 1h.
 * Guards zero/negative deltas → "0m".
 */
 ...
 */
const ACTION_DISPLAY: Partial<Record<string, string>> = {
  Stay: 'Held',
}
...
export function formatSetRow(
  ...
  switch (exercise.type) {
    case 'weighted': {
      ...
      return {
        kind: 'plain',
        text: `Set ${setNumber} — ${weight} × ${actualReps} · ${label}`,
      }
    }
    ...
    case 'bodyweight': {
      return {
        kind: 'plain',
        text: `Set ${setNumber} — ${actualReps}/${targetReps} · ${label}`,
      }
    }
    ...
    case 'time-based': {
      ...
      return {
        kind: 'plain',
        text: `Set ${setNumber} — ${formatTimeBasedDuration(usedSeconds)} · ${label}`,
      }
    }
    ...
    case 'cardio': {
      ...
      return {
        kind: 'plain',
        text: `${formatCardioDuration(seconds)} · ${label}`,
      }
    }
  }
}
