...
export class SessionNotCompleted extends DataLayerError {
  readonly kind = 'forbidden_transition' as const
  readonly sessionId: number
  constructor(sessionId: number) {
    super(`Session ${sessionId} is not completed and cannot be deleted`)
    this.sessionId = sessionId
  }
}

export class SessionHasProgression extends DataLayerError {
  readonly kind = 'forbidden_transition' as const
  readonly sessionId: number
  constructor(sessionId: number) {
    super(
      `Session ${sessionId} has a session_progression and cannot be deleted`,
    )
    this.sessionId = sessionId
  }
}
