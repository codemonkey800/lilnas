...
describe('deleteSession', () => {
  ...
  beforeEach(async () => {
    const { exercises, setLogs: setLogsTable } = await import('src/db/schema')
    exerciseId = testDb.db
      ...
      .returning()
      .get().id
    const session = seedSession({ completedAt: new Date() })
  ...
  it('deletes session and its set_logs for a completed no-progression session', async () => {
    const { deleteSession, getSession } = await import('src/db/sessions')
    const { getSetLogsForSession } = await import('src/db/setLogs')
    await deleteSession({ sessionId })
    expect(await getSession({ id: sessionId })).toBeNull()
    expect(await getSetLogsForSession({ sessionId })).toEqual([])
  })

  it('throws SessionHasProgression and leaves session+logs untouched', async () => {
    const { deleteSession, getSession } = await import('src/db/sessions')
    const { getSetLogsForSession } = await import('src/db/setLogs')
  ...
  it('does NOT throw when session_progression belongs to a different session', async () => {
    const { deleteSession, getSession } = await import('src/db/sessions')
    const { progressions } = await import('src/db/schema')
    // session_progression row for a DIFFERENT session
    const otherSession = seedSession({ completedAt: new Date() })
    testDb.db
      .insert(progressions)
      .values({
        exerciseId,
      ...
      .run()
    // initial/manual_edit rows with null sessionId for this session — must not block
    testDb.db
      .insert(progressions)
      .values({
        exerciseId,
  ...
  it('throws SessionNotCompleted for an active session', async () => {
    const { deleteSession, getSession } = await import('src/db/sessions')
    const activeSession = seedSession()
    await expect(
      deleteSession({ sessionId: activeSession.id }),
    ).rejects.toThrow(/is not completed/)
    expect(await getSession({ id: activeSession.id })).not.toBeNull()
  })

  it('throws NotFoundError for an unknown session id', async () => {
    const { deleteSession } = await import('src/db/sessions')
    await expect(deleteSession({ sessionId: 99999 })).rejects.toThrow(
      /not found/i,
    )
  })
  ...
  it('atomicity: throw after set_logs delete rolls back the whole transaction', async () => {
    ...
    // Perform the real set_logs delete inside the transaction, then throw before
    // the sessions delete. SQLite must roll back the set_logs delete.
    const realDeleteSessionSetLogs = setLogsModule.deleteSessionSetLogs
    const spy = jest
      .spyOn(setLogsModule, 'deleteSessionSetLogs')
      .mockImplementationOnce((executor, sId) => {
        realDeleteSessionSetLogs(executor, sId)
        throw new Error('injected mid-tx failure')
      })
    try {
      await expect(sessionsModule.deleteSession({ sessionId })).rejects.toThrow(
