...
describe('existsCompletedSessionForRoutine', () => {
  ...
  it('history-gate equivalence — agrees with archivedWithHistorySet predicate from stats', async () => {
    ...
    for (const id of [withHistory.id, withoutHistory.id]) {
      expect(await existsCompletedSessionForRoutine({ id })).toBe(
        summaryById.get(id)!.hasHistory,
      )
    }
  })
})
...
describe('deleteRoutine', () => {
  ...
  it('test DB has foreign_keys enabled (restrict tests are real)', () => {
    type WithPragma = { $client: { pragma: (name: string) => unknown } }
    expect(
      (testDb.db as unknown as WithPragma).$client.pragma('foreign_keys'),
