...
const seed = db.transaction(() => {
  ...
  // Every session logs ALL exercises — spec.logs drives progression; exercises
  // absent from spec.logs are logged at their current starting weight with 'Stay'.
  function archivedWeightRoutine(name, days, createdDaysAgo, archivedDaysAgo, exerciseDefs, sessionSpecs) {
    const rId = addRoutine(name, days, createdDaysAgo, archivedDaysAgo)
    const exIds = exerciseDefs.map(([exName, order, sets, reps, startW, inc]) =>
