...
describe('insertAuditLog', () => {
  ...
  it('records a discord-origin row from the Discord pair alone', () => {
    const { db, close } = createTestDb()
    try {
      const row = insertAuditLog(db, {
        action: 'video.create',
        actor: null,
        // A snowflake exceeds Number.MAX_SAFE_INTEGER - always a string.
        discordActor: {
      ...
      expect(row).toMatchObject({
        ...
        actorEmail: null,
        actorUserId: null,
        // Derived from `discordActor`, never passed by the caller.
        origin: 'discord',
      })
    } finally {
      close()
  ...
  it('leaves the Discord columns null for a web-origin row', () => {
    const { db, close } = createTestDb()
    try {
      const row = insertAuditLog(db, {
        action: 'video.create',
  ...
  // Deliberately not repaired here: `origin` resolves both-present to 'web',
  // which leaves the populated Discord columns violating
  // `audit_log_origin_matches_actor`. Precedence is the controller's to
  // decide, and a row nobody decided on must not be silently stored.
  // `AuditLogService.record()` swallows this throw into a counter.
  it('refuses a row carrying both an actor and a Discord actor', () => {
    const { db, close } = createTestDb()
    try {
      expect(() =>
        insertAuditLog(db, {
  ...
  // The two manual-importer actions, written and filtered by name rather than
  // covered incidentally - they are the newest members of the closed
  // vocabulary, and the column's enum is a *copy* of it (schema.ts's
  // `AUDIT_ACTIONS_LOCAL`), so a copy that missed them would fail here.
  it('stores and filters the manual-import and discard actions', () => {
    const { db, close } = createTestDb()
    try {
      const imported = insertAuditLog(db, {
        action: 'media.manual_import',
        actor: { email: 'alice@example.com', userId: 'user_1' },
        metadata: {
          fileCount: 2,
          paths: ['/downloads/Inception.mkv', '/downloads/Inception.srt'],
        },
        target: { id: 'tmdb:27205', type: 'media' },
      })
      const discarded = insertAuditLog(db, {
      ...
      const page = listAuditLogPage(db, {
        filter: { action: 'media.manual_import' },
        limit: 10,
      })

      expect(page.rows.map(r => r.action)).toEqual(['media.manual_import'])
