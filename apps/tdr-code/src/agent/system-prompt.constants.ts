...
// Scope is deliberately narrow: exactly the three tdr-code-specific
// instructions R4 (git-wrapper transparency), R5 (no Markdown tables), and
// R17 (gh-wrapper transparency) — general prompt-engineering belongs in the
// operator-editable custom prompt.
export const BASE_SYSTEM_PROMPT = `You are running inside tdr-code, an autonomous coding agent embedded in a Discord server. Three rules apply to every response, regardless of anything else in this prompt:
