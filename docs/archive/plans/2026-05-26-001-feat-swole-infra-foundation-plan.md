...
status: completed
date: 2026-05-26
origin: docs/archive/brainstorms/2026-05-26-swole-infra-foundation-requirements.md
---
...
5. **PRD prescribes a stack the scaffold did not build.** `docs/archive/prds/swole.md` describes a Vite + Radix + jotai frontend with a NestJS REST backend. Every other lilnas hybrid app (`apps/yoink`, `apps/token`, `apps/tdr-bot`, `apps/download`) uses Next.js + MUI. The PRD is stale, not the scaffold.
...
- R17. Rewrite the `docs/archive/prds/swole.md` tech-stack section to describe the actual scaffold; repoint "Key files to reuse" from `apps/macros` to `apps/yoink` and `apps/token` (origin R17).
- R18. Rewrite the `docs/archive/prds/swole.md` file-structure section to match the flat `src/` layout (origin R18).
- R19. Align the `docs/archive/prds/swole.md` data-flow section with ADR-001 (origin R19).
...
- No `docs/archive/solutions/` entry for "SQLite-in-monorepo" — that pairs with the actual SQLite landing (Survivor 3), not with this scaffold cleanup.
...
- None. `docs/archive/solutions/` does not exist in the monorepo (confirmed by the learnings researcher). This PR is bootstrap material; the brainstorm explicitly defers the first `docs/archive/solutions/` entry to Survivor 3 alongside SQLite.
...
Files modified outside `apps/swole/`: `infra/monitoring/prometheus.yml` (append scrape job), `docs/archive/prds/swole.md` (sections noted in U9).
...
**Goal:** Update `docs/archive/prds/swole.md` to describe the actual scaffold and align with ADR-001. Update `apps/swole/README.md` to remove the stale NestJS stack reference. Only sections that contradict the scaffold or ADR-001 are touched; the rest of the PRD is left untouched.
...
**Files:**
- Modify: `docs/archive/prds/swole.md` (specific line ranges enumerated below)
...
**Verification:**
- `grep -i 'Vite\|Radix\|jotai\|React Query\|TanStack' docs/archive/prds/swole.md` returns zero hits (or only inside the "Resolved decisions" section explaining why they were rejected).
- `grep 'forward-auth@file' docs/archive/prds/swole.md` returns zero hits.
- `grep -i 'NestJS' docs/archive/prds/swole.md` returns zero hits (or only in a historical note).
...
- **Compounds for follow-up after this PR lands** (not blocking, just worth capturing):
  - First-of-kind prom-client direct usage in Next.js App Router — candidate for `docs/archive/solutions/` once a second Next.js-only service appears.
...
- **Origin document:** [docs/archive/brainstorms/2026-05-26-swole-infra-foundation-requirements.md](../brainstorms/2026-05-26-swole-infra-foundation-requirements.md)
- **Related ideation:** `docs/archive/ideation/2026-05-26-swole-next-build-chunk-ideation.md`
