...
status: completed
date: 2026-06-27
origin: docs/archive/brainstorms/2026-06-27-tdr-code-stop-clear-requirements.md
deepened: 2026-06-27
---
...
Today the only way to drive a tdr-code agent is by @mentioning the bot. Messages sent while the agent is working are queued and run sequentially against the same context (see origin: `docs/archive/brainstorms/2026-06-27-tdr-code-stop-clear-requirements.md`). This leaves two gaps:
...
`docs/archive/solutions/` has **no domain-direct entries** (no Discord/Necord/ACP/child-process precedent — tdr-code is the repo's newest app). Three cross-domain analogies inform the hard parts:

- **`docs/archive/solutions/conventions/begin-immediate-for-read-then-write-mutations-2026-05-27.md`** — read-then-write races need a serialization point taken *before* the read. Applied here: the cancel path and the `finally`-drain both touch session/queue state. The resolution is to make the synchronous queue-clear in `cancel()` provably happen-before the drain (see Key Technical Decisions #2) rather than locking — Node's single-threaded loop gives us the ordering for free.
...
- **`docs/archive/solutions/ui-bugs/drawer-history-marker-repush-on-keystroke-2026-05-30.md`** + **`docs/archive/solutions/conventions/atomicity-tests-must-reach-the-write-phase-2026-06-03.md`** — every resource acquired at turn-start must be released on *every* termination path (complete / error / stop / clear), and race tests must inject the contended event inside the real window (not before it) or they pass vacuously. Both shape U1/U2 test scenarios.
...
- **Origin document:** [docs/archive/brainstorms/2026-06-27-tdr-code-stop-clear-requirements.md](docs/archive/brainstorms/2026-06-27-tdr-code-stop-clear-requirements.md)
- Session lifecycle: `apps/tdr-code/src/agent/session-manager.service.ts`
