...
- **Pair with atomicity tests.** The pattern is committed alongside tests that simulate concurrent writers (commit `4a76e7e feat(swole): add transactional mutations with BEGIN IMMEDIATE and atomicity tests`). Tests live in `apps/swole/src/db/__tests__/` and lock the contract in. Writing one that *genuinely* proves rollback (rather than passing vacuously on a guard-refusal path) has its own pitfalls — see [`atomicity-tests-must-reach-the-write-phase-2026-06-03.md`](./atomicity-tests-must-reach-the-write-phase-2026-06-03.md).
...
- [`docs/archive/solutions/conventions/type-guards-over-nonnull-assertions-on-db-rows-2026-05-30.md`](./type-guards-over-nonnull-assertions-on-db-rows-2026-05-30.md) — the sibling `swole` data-layer convention. Generalizes the same discriminated-union technique as the `DataLayerError` `kind` hierarchy above, applied to nullable DB columns; the `isNotNull`-guaranteed `CompletedSessionLogEntry` type lives in the same `setLogs.ts` that uses this `IMMEDIATE` convention.
- [`docs/archive/solutions/conventions/atomicity-tests-must-reach-the-write-phase-2026-06-03.md`](./atomicity-tests-must-reach-the-write-phase-2026-06-03.md) — how to write the atomicity test this convention asks for so it actually exercises rollback, instead of stopping at a pre-write guard and passing vacuously.
