...
- **`tdr-code` stays as it is.** It deliberately left forward-auth for app-owned Discord OAuth
  (`docs/runbooks/tdr-code-phase-d-forward-auth-cutover.md`). It is not migrated onto `lilnas-auth`.
...
- **Admin authorization independent of application data.** Directly informed by
  `docs/runbooks/tdr-code-phase-d-forward-auth-cutover.md` §5.1, which documents that re-adding the Traefik
