...
- *Implementation note:* the embedded history's `GET /history` query doesn't accept type/status parameters yet and needs that extension (the underlying job filter already supports both; the shape belongs in the plan doc, not here). No new access-control surface: filtering stays inside the same self-or-admin requester scoping the history already enforces.
