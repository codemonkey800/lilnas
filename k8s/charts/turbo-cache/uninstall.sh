#!/bin/bash
# Uninstall the turbo-cache Helm chart
...
echo "=== WARNING ==="
echo "This will uninstall the turbo-cache Helm chart and remove all associated resources."
echo ""
...
echo "• Cache data stored in MinIO will remain unless manually deleted"
echo "• Any Turborepo projects using this cache will fall back to local caching"
echo "• TLS certificates may be retained for reuse if the same hostname is used again"
echo ""
echo "To check MinIO cache data:"
echo "  kubectl port-forward -n lilnas-core service/minio 9001:9001"
echo "  # Then visit http://localhost:9001 and check the 'build' bucket"
echo ""
echo "To clean up MinIO cache data (if desired):"
echo "  # Connect to MinIO console and delete contents of 'build' bucket"
echo "  # Or use mc (MinIO Client) to remove cache data"
echo ""
echo "To redeploy turbo-cache:"
echo "  ./deploy.sh"
echo ""
